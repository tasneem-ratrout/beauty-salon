...
public class colorupdatecontroller {
    @FXML
    ...
    @FXML
    void backactionbutten(MouseEvent event) throws IOException {
        App.setRoot("servicemang");
    }
    @FXML
    void pricechangebutten(ActionEvent event) throws IOException {
    	 String newPrice = priceTextArea.getText().trim();
         if (newPrice.isEmpty()) {
             showAlert("Invalid input", "Please enter a valid price.", Alert.AlertType.ERROR);
             return;
         }
         updatePriceInDatabase("COLORING", newPrice);
    }
    @FXML
    void initialize() {
        loadServiceInformation("COLORING");
    }
    ...
    void Descriptionchangedaction(ActionEvent event) throws IOException {
        ...
        updateDescriptionInDatabase("COLORING", newDescription);
    }
    ...
    private void updatePriceInDatabase(String serviceName, String newPrice) {
        ...
        try (Connection connection = DatabaseConnection.connect();
            ...
            preparedStatement.setString(2, serviceName); // اسم الخدمة
